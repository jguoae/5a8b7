...
int main(){
  ...
  #pragma omp parallel private(c,d,N)
  {
    for(;;){
      ...
      if(!bfs.empty()){
        // cout<<count<<dfs.size()<<"\n";
        c = bfs.front().firstvalue;
        d = bfs.front().secondvalue;
        double gc = bfs.front().g_f;
        double gd = bfs.front().g_s;
        N=M;
        count++;
        bfs.pop();
        omp_unset_lock(&lock);
        ...
        if(result <= N+epi){
          ...
          omp_set_lock(&lock);
          M=N;
          bfs.push(new_node(c,mid_value,result_c,100));
          bfs.push(new_node(mid_value,d,100,result_d));
          count--;
          omp_unset_lock(&lock);
        }
      }
